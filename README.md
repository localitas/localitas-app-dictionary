...
- Dictionary API lookups with phonetics and examples
- Urban Dictionary as alternative source
- Configurable lookup sources (--sources flag)
...
```bash
# Clone the repo
git clone https://github.com/localitas/localitas-app-dictionary.git ~/localitas-app-dictionary
...
```bash
cd ~/localitas-app-dictionary

# Build and run locally
make build
./bin/dictionary-server serve --listen :8000
...
3. This app is available at `https://{your-subdomain}.localitas.com/apps/ext/dictionary/`
...
```bash
localitas-core app-store add --name dictionary --compose ./docker-compose.yml --port 9205
localitas-core app-store start dictionary
```

Or open the App Store UI (package icon, top-right nav) and paste the `docker-compose.yml`.

The image is published to `ghcr.io/localitas/localitas-app-dictionary:latest`. To publish a new version:
